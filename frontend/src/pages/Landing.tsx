...
const Landing = () => {
  ...
  const heroFontSize = isMobile ? 32 : isTablet ? 44 : 64;
  const heroPFontSize = isMobile ? 15 : 18;
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .landing-waves > use { animation: wave-move 20s linear infinite; }
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 1, height: '40vh', pointerEvents: 'none' }}>
        <svg viewBox="0 24 150 28" preserveAspectRatio="none" style={{ width: '100%', height: '100%', opacity: 0.8 }}>
          <defs>
      ...
      <div style={{ position: 'absolute', bottom: '15%', right: '20%', width: 400, height: 400, background: 'radial-gradient(circle, rgba(121,192,255,0.04) 0%, transparent 70%)', borderRadius: '50%', pointerEvents: 'none' }} />
      ...
      <nav style={{
        ...
        maxWidth: 1400, margin: '0 auto', flexWrap: 'nowrap', gap: 12,
      }}>
        {/* Logo */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: isMobile ? 16 : 18, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 16, flexShrink: 0 }}>
          <button
            ...
            style={{
              ...
              borderRadius: 30, background: 'transparent', color: theme.textMain,
              fontWeight: 600, fontSize: isMobile ? 13 : 14,
              border: '1px solid #2E3640', cursor: 'pointer', transition: 'all 0.2s',
              whiteSpace: 'nowrap',
            }}
            onMouseEnter={e => { e.currentTarget.style.background = '#1F2933'; }}
      ...
      <main style={{
        ...
        <>
          <motion.div
            initial={{ opacity: 0, x: -50, rotate: -15, y: 0, scale: isMobile ? 0.7 : 1 }}
            animate={{ opacity: 1, x: 0, rotate: -10, y: [0, -25, 0], scale: isMobile ? 0.7 : 1 }}
            transition={{
              opacity: { duration: 0.8 }, x: { duration: 0.8 }, rotate: { duration: 0.8 },
              y: { duration: 6, repeat: Infinity, ease: 'easeInOut', delay: 0 },
            }}
            style={{
              position: 'absolute', 
              left: isMobile ? '-10%' : isTablet ? '3%' : '8%', 
              top: isMobile ? '30px' : '120px',
              background: theme.cardBg, border: `1px solid ${theme.cardBorder}`,
              borderRadius: 16, padding: 24, width: isTablet ? 170 : 220,
          ...
          <motion.div
            ...
            animate={{ opacity: 1, x: 0, rotate: 10, y: [0, -25, 0], scale: isMobile ? 0.7 : 1 }}
            transition={{
              opacity: { duration: 0.8, delay: 0.1 }, x: { duration: 0.8, delay: 0.1 }, rotate: { duration: 0.8, delay: 0.1 },
              y: { duration: 6, repeat: Infinity, ease: 'easeInOut', delay: 2 },
            }}
            style={{
              position: 'absolute', 
              right: isMobile ? '-10%' : isTablet ? '3%' : '8%', 
              top: isMobile ? '400px' : '140px',
              background: theme.cardBg, border: `1px solid ${theme.cardBorder}`,
              borderRadius: 16, padding: 24, width: isTablet ? 170 : 220,
              boxShadow: '0 20px 40px rgba(0,0,0,0.4)',
              zIndex: 5,
        ...
        >
          <h1 style={{
            ...
            {isMobile ? (
              <>Experience seamless orchestration with smart agents made for{' '}
                <span style={{ background: theme.accent, color: '#000', padding: '2px 12px', borderRadius: 999, display: 'inline-block' }}>
                  modern teams
                </span>
              </>
            ) : (
              <>
                ...
                agents<br />
                made for{' '}
                <span style={{ background: theme.accent, color: '#000', padding: '4px 20px', borderRadius: 999, display: 'inline-block', transform: 'translateY(4px)' }}>
                  modern teams
                </span>
              </>
            )}
          ...
          <div style={{
            ...
            <button
              onClick={() => navigate('/login')}
              style={{
                padding: isMobile ? '14px 0' : '16px 32px',
