...
const inputStyle = (isMobile: boolean): React.CSSProperties => ({
  ...
  fontSize: isMobile ? 16 : 14, // 16px prevents iOS auto-zoom
  outline: 'none',
        ...
        <div style={{
          ...
          {pendingVerification ? (
            <form onSubmit={handleVerify}>
              <h2 style={{ fontSize: isMobile ? 20 : 22, fontWeight: 600, margin: '0 0 8px', textAlign: 'center' }}>Verify your email</h2>
              <p style={{ color: theme.textMuted, fontSize: 13, margin: '0 0 24px', textAlign: 'center' }}>
                We sent a code to <strong style={{ color: theme.textMain }}>{email}</strong>.
              </p>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <input
                  ...
                  style={{ ...inputStyle(isMobile), textAlign: 'center', letterSpacing: 6, fontSize: isMobile ? 20 : 18 }}
                />
              </div>
              <button
            ...
            <>
              <h2 style={{ fontSize: isMobile ? 20 : 22, fontWeight: 600, margin: '0 0 8px', textAlign: 'center' }}>Create your account</h2>
              <p style={{ color: theme.textMuted, fontSize: 13, margin: '0 0 24px', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexDirection: isMobile ? 'column' : 'row' }}>
                <button
                  type="button" onClick={() => handleOAuth('oauth_google')}
                  style={{
                    flex: 1, padding: '12px', borderRadius: 12,
                    background: theme.buttonBg, border: `1px solid ${theme.border}`,
                ...
                <button
                  type="button" onClick={() => handleOAuth('oauth_github')}
                  style={{
                    flex: 1, padding: '12px', borderRadius: 12,
                    background: theme.buttonBg, border: `1px solid ${theme.border}`,
                    color: theme.textMain, fontSize: 13, fontWeight: 600,
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexDirection: isMobile ? 'column' : 'row' }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: theme.textMain, marginBottom: 8 }}>First name</label>
                    <input type="text" required value={firstName} onChange={e => setFirstName(e.target.value)} style={inputStyle(isMobile)} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: theme.textMain, marginBottom: 8 }}>Last name</label>
                    <input type="text" required value={lastName} onChange={e => setLastName(e.target.value)} style={inputStyle(isMobile)} />
                  </div>
                </div>

                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: theme.textMain, marginBottom: 8 }}>Email address</label>
                  <input type="email" required autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} style={inputStyle(isMobile)} />
                </div>

                <div style={{ marginBottom: 24 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: theme.textMain, marginBottom: 8 }}>Password</label>
                  <input type="password" required autoComplete="new-password" value={password} onChange={e => setPassword(e.target.value)} style={inputStyle(isMobile)} />
                </div>
